...
%% 2) PARAMETERS FOR NEW SCHEME: E[|H_bob|^2] = beta E[|H_eve|^2]

beta_db = 10;
beta = 20^(beta_db/10);                                     % Relative power between bob and eve's channels
...
switch scenario 
    ...
    case "ber_ebno_bor_alpha"
        nb_block    = 10;          % Need lot of blocks
        BOR         = [2 4 8];      % Back Of Rate (upsampling factor)
        EbN0        = [-5:20];      % Energy per bit to noise power spectral density ratio
        alpha       = .95;           % Percentage to the useful data (--> 1-alpha corresponds to the energy used for the AN)
