...
W = W/sqrt(energy_W)*sqrt(energy_sym_useful_TX);
energy_W_final = sum(abs(W).^2)/nb_subcar;
...
[sym_RX_noisy_bob,noise_power_bob] = TR_security_addNoise(sym_RX_bob, EbN0(nn), M, var(sym_raditated_noiseless));   % Noisy received symbols (useful + AN) @ Bob
[sym_RX_noisy_eve,noise_power_eve] = TR_security_addNoise(sym_RX_eve, EbN0(nn), M, var(sym_raditated_noiseless));   % Noisy received symbols (useful + AN) @ Eve
...
% useful symbol
energy_sym_useful_RX_despread_bob = mean(var(sym_useful_RX_despread_bob)) ; %mean(1/nb_subcar*sum(abs(sym_useful_RX_despread_bob).^2)); %mean(var(sym_useful_RX_despread_bob)) ; %mean(1/nb_subcar*sum(abs(sym_useful_RX_despread_bob).^2));
energy_sym_useful_RX_despread_eve = mean(var(sym_useful_RX_despread_eve)) ; %mean(1/nb_subcar*sum(abs(sym_useful_RX_despread_eve).^2));

% AN symbols (only @ Eve)
energy_sym_AN_RX_despread_eve =  var(sym_AN_RX_despread_eve) ; % 1/nb_subcar*sum(abs(sym_AN_RX_despread_eve).^2); % %
an_energy(cc,nn,nb_bor,aa) = energy_sym_AN_RX_despread_eve;
...
SINR_eve_despread(cc,nn,nb_bor,aa) = energy_sym_useful_RX_despread_eve/(energy_noise_despread_eve+energy_sym_AN_RX_despread_eve); %(mean(noise_power_eve)+energy_sym_AN_RX_despread_eve);% (energy_noise_despread_eve+energy_sym_AN_RX_despread_eve);
test(cc,nn,nb_bor,aa) = energy_sym_useful_RX_despread_bob;
test2(cc,nn,nb_bor,aa) = energy_noise_despread_bob;
test3(cc,nn,nb_bor,aa) = energy_noise_despread_eve;
...
energy_noise_despread_bob_eq = mean(1/nb_subcar*sum(abs(noise_despread_bob_eq).^2));
...
% AN symbols (only @ Eve)
energy_sym_AN_RX_despread_eve_eq = 1/nb_subcar*sum(abs(sym_AN_RX_despread_eve_eq).^2);
...
%% tmp
an_energy = squeeze(mean(an_energy,1));
signal_power_one_subcar_bob = squeeze(mean(signal_power_one_subcar_bob,1));
...
if (sc(scenar) == 'secrecy_alpha_ebno_bor' || sc(scenar) == 'secrecy_alpha_bor_ebno')
    ...
    for nb_bor = 1:length(BOR)
    for q = 0:D
        sum_term(nb_bor,q+1) =  LambdaFunc(BOR(nb_bor), q, D) .* gamma(q+3);
    end
    end
    sum_term = sum(sum_term,2);
    A = sum_term./(BOR'.^2.*gamma(BOR').*2.^(BOR'+3));
    SINR_eve_theoretical = (4*alpha.*A) ./ (an_energy + noise_power_one_subcar_eve); % 1-alpha already taken into account
    SINR_bob_theoretical = alpha.*(BOR'+1).*BOR'./(noise_power_one_subcar_bob.*BOR'.^2);
...
if (sc(scenar) == 'secrecy_alpha_ebno_bor' || sc(scenar) == 'secrecy_alpha_bor_ebno')
an_energy = an_energy./(1-alpha);                                 % To have Energy AN / (1-alpha) constant
sigma_an = an_energy;
sigma_b = noise_power_one_subcar_bob;
