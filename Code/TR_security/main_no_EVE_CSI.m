...
if (sc(scenar) == 'secrecy_alpha_ebno_bor' || sc(scenar) == 'secrecy_alpha_bor_ebno')
    ...
    SINR_eve_theoretical = alpha./BOR' ./ ((1-alpha).*energy_an  + energy_noise_eve_one_subcar); %(4*alpha.*A) ./ ((1-alpha).*energy_an  + energy_noise_eve_one_subcar); % 1-alpha already taken into account
...
% T1 = BOR'.*(BOR'+1).*sigma_an;
% T2 = (BOR'+1).*(BOR'.*sigma_an + BOR'.*sigma_e);
% T3 = BOR'.*sigma_b.*(BOR'.*sigma_an + BOR'.*sigma_e);
% T4 = (1-BOR'.*sigma_an).*BOR'.*sigma_b;



T1 = BOR'.*(BOR'+1).*sigma_an;
T2 = BOR'.*(BOR'+1).*(sigma_an + sigma_e)-BOR'.^2.*sigma_b.*sigma_an;
T3 = BOR'.*(sigma_b+sigma_an +sigma_e);
T4 = (1-BOR'.*sigma_an).*BOR'.*sigma_b;
...
% Analytic results
energy_an_max_secrecy = 100*(1-(sqrt(T1(:,1).^2.*T3(:,1).^2 + T1(:,1).*...
    T2(:,1).*T3(:,1).*T4(:,1) - T1(:,1).*T3(:,1).*...
    T4(:,1).^2) - T1(:,1).*T3(:,1)) ./ (T1(:,1).*T4(:,1)));
end
plot_result;
% save_figure;
...
figure;
plot( 100*alpha', secrecy_capa_theoretical,'r'); hold on;%log2( (-alpha.^2.*T1 + alpha.* T2 + T3) ./ (alpha.*T4 + T3 ) ),'r'); hold on;
plot(100*alpha', secrecy_capa_ergo.','g') ; hold on;
xlabel('Percentage of energy radiated dedicated for useful data (\%)')
...
alpha_global = 1-energy_an_max_secrecy./100;
save 'alpha.mat'  'alpha_global'
save 'secrecy_max.mat' 'max_value_theoretical' 'max_value'
