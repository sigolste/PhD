...
%
%
%   Introduction and modelization of Eve subcarrier correlatin. In this
%   code, we assume that both Bob and Eve have independant subcarrier
%   correlation. Modelization of their capa (SDS, MF, OC decoders for Eve) 
%   It may be the case that the first ordrer approximation of Bob and capa 
%   can compensante to have a correct approximation of the SR.
%
%
%   Code Started:   01.03.2021
%   Last Update:    01.03.2021
%
%  
%   Mathematical derivations: cfr FC38 verso - 
%
%  © SIDNEY GOLSTEIN
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
sigma_tau_e = .5e-6 ;                                         % Delay spread (3us = urban ,  .5us = suburban, .2us = open areas)
...
sinr1_b_model  = zeros(length(alpha),size(b_subcar_b,2),size(b_subcar_e,2) ,length(snr_b),length(snr_e));
sinr1_e_model  = zeros(length(alpha),size(b_subcar_b,2),size(b_subcar_e,2) ,length(snr_b),length(snr_e));
sinr2_e_model  = zeros(length(alpha),size(b_subcar_b,2),size(b_subcar_e,2) ,length(snr_b),length(snr_e));
sinr5_e_model  = zeros(length(alpha),size(b_subcar_b,2),size(b_subcar_e,2) ,length(snr_b),length(snr_e));

% Channel Generation:
for bb = 1:length(U)
    for dd = 1:size(b_subcar_b,2) % Bob
        [Hb(:,:,bb,dd), Hb1(:,:,bb,dd), abs_rho(:,bb,dd), Tb(:,:,bb,dd)] = corr_frequency( Q , b_subcar_b(bb,dd) , sigma_tau_b , nb_run ) ;
    end
    for dd = 1:size(b_subcar_e,2) % Eve
        [He(:,:,bb,dd), He1(:,:,bb,dd), ~ , Te(:,:,bb,dd)] = corr_frequency( Q , b_subcar_e(bb,dd) , sigma_tau_e , nb_run ) ;
    end
end
...
for bb = 1:length(U)
    for dd = 1:size(b_subcar_b,2)
        for ee = 1:size(b_subcar_e,2)
            for nn = 1:length(snr_b)
                for mm = 1:length(snr_e)
                    sinr1_b_model(:,bb,ee,dd,nn)    = sinrModelingFrequencyCorrelation(alpha,U(bb),N(bb),Tb(:,:,bb,dd),Te(:,:,bb,ee),snr_b(nn),snr_e(mm),"bob_correl").';
                    sinr1_e_model(:,bb,ee,dd,nn)    = sinrModelingFrequencyCorrelation(alpha,U(bb),N(bb),Tb(:,:,bb,dd),Te(:,:,bb,ee),snr_b(nn),snr_e(mm),"eve_decod1_correlE_correlB").';
                    sinr2_e_model(:,bb,ee,dd,nn)    = sinrModelingFrequencyCorrelation(alpha,U(bb),N(bb),Tb(:,:,bb,dd),Te(:,:,bb,ee),snr_b(nn),snr_e(mm),"eve_decod2_correlE_correlB").';
                end
            end
        end
    end
...
capa1_b_model = log2(1+sinr1_b_model);
